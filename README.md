...
То, что пока явно не указано в задании, но уже хотелось бы добавить

- [ ] простая валидация полей: пусто / не пусто (пока не разобрался как это сделать без `if/esle`'ов с массивами на входе)
